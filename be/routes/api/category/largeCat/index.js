...
router.put('/:id', (req, res, next) => {
  ...
  console.log(_id)
  LargeCat.findOne({ _id })
    .then((r) => {
      if (!r) return next(createError(400, '카테고리가 없습니다'))
      return LargeCat.updateOne({ _id }, { $set: req.body })
    })
    .then(r => {
      res.send({ success: true, msg: r });
    })
...
router.delete('/:id', (req, res, next) => {
  const _id = req.params.id;
  Post.deleteMany({ _largeCat: _id })
    .then(r => {
      if (!r) return new Promise(null)
      return SmallCat.deleteMany({ _largeCat: _id })
    })
    .then(r => {
      if (!r) return new Promise(null)
      return LargeCat.deleteOne({ _id })
    })  
    .then(r => {
      console.log(r)
      res.send({ success: true, msg: r });
